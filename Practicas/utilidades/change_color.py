...
def show_menu(screen):
    ...
    while True:
        screen.clear()
        screen.addstr(0,0,'Mi programa - Menú Principal', curses.A_BOLD)
        #Mostrar el menú con colores
        for i, option in enumerate(options):
            if i==selection:
                screen.addstr(i+2,5,option,curses.color_pair(1))
            else:
        ...
        if key==curses.KEY_UP and selection>0:
            ...
            if selection==len(options)-1:
                ...
                screen.addstr(len(options),0,f'Has elegido {options[selection]}. Presiona Enter para continuar...',curses.color_pair(2))
                screen.getch()
